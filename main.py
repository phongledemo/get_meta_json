...
from config import username, userInput_id, passwordInput_id, password, DNAC_Shockwave_Solution, metajson_xpath, login_button_id,\
    trust_browser_button_id, tbody_xpath_1, tbody_xpath_2, information_metajson_xpath, DNAC_Guardian_Solution, DNAC_Ghost_Solution, DNAC_Groot_Solution, log, try_again_loading, database, big_table
...
def debug_decorator(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if type(args[0]) == list and len(args[0]) > 20:
            logger.debug(f'Entering function : {func_name} with args: list len {len(args[0])} and {tuple(list(args)[1:])} kwargs {kwargs}')
        else:
...
if __name__ == "__main__":
    ...
    modes = ["shockwave", "guardian", "groot", "ghost"]
